        ...
        with gr.Blocks() as demo:
            ...
            with gr.Tabs():
                ...
                with gr.TabItem("JSON格式化"):
                    gr.Markdown("## JSON格式化")
                    with gr.Row():
                        input_json_text = gr.Textbox(lines=10, label="输入 JSON")
                        output_json_text = gr.Textbox(lines=10, label="格式化 JSON")
                    format_button = gr.Button("格式化").click(
                        format_json, inputs=[input_json_text], outputs=output_json_text
                    )
